...
def grammer_parser():
	if request.method == 'POST':
		...
		elif(request.form['parse_type'] == 'clr_i'):
			input_grammar = request.form['grammar']
			write_to_file = open("temp_rules.txt", 'w')
			write_to_file.write(input_grammar)
			write_to_file.close()
			os.system('python3 lr1.py i')
			time.sleep(1)
			
			try:
				read_from_file = open("temp_clr_items.txt", 'r').read()
			except:
				return render_template('index.html', title = "Output comes here:", output = "", entered_grammar=input_grammar[:], error_code = 1)
			return render_template('index.html', title = "CLR Items", output = read_from_file, entered_grammar=input_grammar[:], error_code = 0)

		elif(request.form['parse_type'] == 'clr_t'):
			input_grammar = request.form['grammar']
			write_to_file = open("temp_rules.txt", 'w')
			write_to_file.write(input_grammar)
			write_to_file.close()
			os.system('python3 lr1.py t')
			time.sleep(1)
			try:
				read_from_file = open("temp_clr_table.txt", 'r').read()
			except:
				return render_template('index.html', title = "Output comes here:", output = "", entered_grammar=input_grammar[:], error_code = 1)
			return render_template('index.html', title = "CLR Table", output = read_from_file, entered_grammar=input_grammar[:], error_code = 0)
